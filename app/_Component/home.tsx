...
function HomePage(props: { handleData: (data: string[]) => Promise<Video[]> }) {
  //创建多个列表，用于存储用户选择的食材
  ...
  React.useEffect(() => {
    props.handleData(result.map((item) => item.bv) as string[]).then((data) => {
      setVideoData(data);
    });
  }, [result, props]);
  /**头部标签 */
  ...
  return (
    <div className="flex flex-col items-center w-full gap-y-4 pt-2">
      ...
      {modeTags}
      <p>你要先选食材或工具哦～</p>
      <RecipeCard recipe={result} VideoData={videoData} />
    </div>
  );
}
